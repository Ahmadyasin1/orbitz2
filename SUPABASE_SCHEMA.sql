-- =============================================
-- ORBITZ TECHNOLOGY DATABASE SCHEMA
-- =============================================
-- Run this SQL in your Supabase SQL Editor
-- Project: salman's Project
-- URL: https://wcoqnrwifybuhcgeasep.supabase.co
...
-- Authenticated admin write access only (service role always bypasses RLS)
DROP POLICY IF EXISTS "Admin full access for blogs" ON blogs;
DROP POLICY IF EXISTS "Admin full access for case_studies" ON case_studies;
DROP POLICY IF EXISTS "Admin full access for whitepapers" ON whitepapers;
DROP POLICY IF EXISTS "Admin full access for webinars" ON webinars;
DROP POLICY IF EXISTS "Admin full access for admin_users" ON admin_users;

-- Allow authenticated admins to manage content (match by email in admin_users)
CREATE POLICY "Admin write blogs" ON blogs
  FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
CREATE POLICY "Admin update blogs" ON blogs
  FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email())) WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
CREATE POLICY "Admin delete blogs" ON blogs
  FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));

CREATE POLICY "Admin write case_studies" ON case_studies
  FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
...
CREATE POLICY "Admin write whitepapers" ON whitepapers
  FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
CREATE POLICY "Admin update whitepapers" ON whitepapers
  FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email())) WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
CREATE POLICY "Admin delete whitepapers" ON whitepapers
  FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));

CREATE POLICY "Admin write webinars" ON webinars
  FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
CREATE POLICY "Admin update webinars" ON webinars
  FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email())) WITH CHECK (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
CREATE POLICY "Admin delete webinars" ON webinars
  FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users au WHERE au.email = auth.email()));
...
-- Sample Blogs
INSERT INTO blogs (title, slug, excerpt, content, featured_image_url, author_name, tags, read_time, views, likes) VALUES
(
...
-- Sample Case Studies
INSERT INTO case_studies (title, slug, excerpt, content, featured_image_url, client_name, industry, project_duration, project_budget, technologies, results, views) VALUES
(
...
-- Sample Whitepapers
INSERT INTO whitepapers (title, slug, excerpt, content, featured_image_url, author_name, downloads, tags) VALUES
(
...
-- Sample Webinars
INSERT INTO webinars (title, slug, excerpt, content, featured_image_url, duration, presenter_name, presenter_title, scheduled_at, registrations, tags) VALUES
(
...
SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('blogs', 'case_studies', 'whitepapers', 'webinars', 'admin_users');

-- Check sample data
SELECT COUNT(*) as blog_count FROM blogs;
SELECT COUNT(*) as case_study_count FROM case_studies;
SELECT COUNT(*) as whitepaper_count FROM whitepapers;
SELECT COUNT(*) as webinar_count FROM webinars;

-- =============================================
-- SETUP COMPLETE!
-- =============================================
-- Your database is now ready for the Orbitz Technology website
-- You can now use the admin panel at /admin to manage content
-- The website will automatically load content from these tables
